...
[ApiVersion("1.0")]
[Authorize]
public sealed class CustomersControllers(ISender sender) : ApiController
{
    [HttpGet]
    ...
    [EndpointDescription("Returns detailed information about a customer if found.")]
    ...
    {
        var vehicles = request.Vehicles
            .ConvertAll(vehicle => new UpdateVehicleCommand(
                vehicle.VehicleId, vehicle.Make, vehicle.Model, vehicle.Year, vehicle.LicensePlate
        ));

        var result = await sender.Send(new UpdateCustomerCommand(
            customerId, request.Name, request.PhoneNumber, request.Email, vehicles),cancellationToken);
