...
public sealed class DashboardController(ISender sender) : ApiController
{
    ...
    public async Task<IActionResult> GetTodayStats([FromQuery] DateOnly? date, CancellationToken cancellationToken)
    {
        var startDate = date ?? DateOnly.FromDateTime(DateTime.UtcNow);
        var result = await sender.Send(new GetWorkOrderStatsQuery(startDate), cancellationToken);
