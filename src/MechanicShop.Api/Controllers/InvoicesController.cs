...
public sealed class InvoicesController(ISender sender) : ApiController
{
    ...
    public async Task<IActionResult> IssueInvoice(Guid workOrderId, CancellationToken cancellationToken)
    {
        ...
        return result.Match(
            response => CreatedAtAction(nameof(GetInvoice), new {version = "1.0", invoiceId = response.InvoiceId}),
            Problem
        );
    }
    ...
    [MapToApiVersion("1.0")]
    public async Task<IActionResult> GetInvoice(Guid workOrderId, CancellationToken cancellationToken)
    {
        var query = new GetInvoiceByIdQuery(workOrderId);
