...
public sealed class RepairTasksController(ISender sender) : ApiController
{
    ...
    [MapToApiVersion("1.0")]
    public async Task<IActionResult> Create([FromBody] CreateRepairTaskCommand request, CancellationToken cancellationToken)
    {
        var parts = request.Parts
            .ConvertAll(part => new CreateRepairTaskPartCommand(
                part.Name, part.Cost, part.Quantity));
    ...
    {
        var parts = request.Parts
            .ConvertAll(part => new UpdateRepairTaskPartCommand(
                part.PartId, part.Name, part.Cost, part.Quantity))
;
