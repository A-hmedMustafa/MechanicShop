...
public class ApiService(IHttpClientFactory httpClientFactory, TimeZoneService timeZoneService)
{
    ...
    public async Task<ApiResult<List<CustomerModel>>> GetCustomersAsync()
    {
        ...
        {
            return await HandleExceptionAsync<List<CustomerModel>>(ex, "Failed to fetch customers.");
        }
    }
    ...
    public async Task<ApiResult<CustomerModel>> GetCustomerAsync(Guid customerId)
    {
        ...
        {
            return await HandleExceptionAsync<CustomerModel>(ex, $"Failed to fetch customer with Id {customerId}.");
        }
    } 
    ...
    public async Task<ApiResult<PaginatedList<WorkOrderListItemModel>>> GetWorkOrdersAsync(
        WorkOrderFilterRequest filterRequest,
    ...
    )
    {
        try
        {
            var queryString = BuildQueryString(filterRequest, pageRequest);
        ...
        {
            return ApiResult<PaginatedList<WorkOrderListItemModel>>.Failure("Request was canceled.");
        }
        catch (Exception ex)
        {
    ...
    public async Task<ApiResult> UpdateWorkOrderLaborAsync(Guid workOrderId, AssignLaborRequest request)
    {
        try
        {
            var response = await _httpClient.PutAsJsonAsync($"api/v1/workorders/{workOrderId}/assign-labor", request);
        ...
        {
            return await HandleExceptionAsync(ex, $"Failed to update labor assignment for work order {workOrderId}");
        }
    }
    ...
    public async Task<ApiResult<ScheduleModel>> GetDailyScheduleAsync(DateOnly date, Guid? laborId = null)
    {
        try
        {
            ...
            if (response.IsSuccessStatusCode)
            {
                ...
                if(schedule is null)
                {
                    return ApiResult<ScheduleModel>.Failure("Schedule data is empty.");
                }
                try
                {
                    ...
                    {
                        slot.StartsAt = TimeZoneInfo.ConvertTime(slot.StartsAt, timeZone);
                        slot.EndsAt = TimeZoneInfo.ConvertTime(slot.EndsAt, timeZone);
                    }
                }
    ...
    public async Task<ApiResult> SettleInvoiceAsync(Guid invoiceId)
    {
        try
        {
    ...
    private static async Task<ApiResult<T>> HandleErrorResponseAsync<T>(HttpResponseMessage response)
    {
        ...
        if(problemDetails is not null)
        {
            return ApiResult<T>.Failure(
                ...
                details: problemDetails.Detail ?? "Error",
