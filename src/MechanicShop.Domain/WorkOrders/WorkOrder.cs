...
public sealed class WorkOrder : AuditableEntity
{
    ...
    public decimal? TotalPartsCost => _repairTasks
        .SelectMany(partsList => partsList.Parts)
        .Sum(part=>part.Cost);
    ...
    public Result<Updated> AddRepairTask(RepairTask repairTask)
    {
        ...
        if(_repairTasks.Any(r=>this.Id == repairTask.Id))
        {
    ...
    public Result<Updated> UpdateTiming(DateTimeOffset startsAt, DateTimeOffset endsAt)
    {
        if(!IsEditable)
        {
    ...
    public bool CanTransitionTo(WorkOrderState newStatus)
    {
        return (State, newStatus) switch
        {
            ...
            (_, WorkOrderState.Cancelled) when State is not WorkOrderState.Completed => true,
            _ => false            
        };
    }
