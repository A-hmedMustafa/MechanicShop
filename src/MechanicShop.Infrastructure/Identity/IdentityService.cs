...
) : IIdentityService
{
    ...
    public async Task<bool> IsInRoleAsync(string userId, string role)
    {
        ...
        return user is null ? false : await _userManager.IsInRoleAsync(user, role);
    }
    ...
    public async Task<Result<AppUserDto>> AuthenticateAsync(string email, string password)
    {
        ...
        if(!user.EmailConfirmed)
            return Error.Conflict("Email_Not_Confirmed", $"Email {UtilityService.MaskEmail} not confirmed.");
    
        if(!await _userManager.CheckPasswordAsync(user, password))
            return Error.Conflict("Invalid_Login_Attempt", "Email / Password is incorrect.");
